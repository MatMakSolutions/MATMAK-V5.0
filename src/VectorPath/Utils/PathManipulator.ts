...
import { SvgCommand, TSVGCommand } from "./Commands/SvgCommand";
...
export class PathManipulator {
  ...
  calculatePreviousPoints() {
    ...
    for (let i = 0; i < this.data.length; i++) {
      ...
      if (cmd.type === 'M') {
        ...
        (cmd as LCommand).startingPoint = previousPoint;
        previousPoint = { x: (cmd as LCommand).x, y: (cmd as LCommand).y };
      } else if (cmd.type === 'C') {
        (cmd as CCommand).previous = previousPoint;
        previousPoint = { x: (cmd as CCommand).x, y: (cmd as CCommand).y };
      }
    }
    if (isRelative) {
  ...
  public translate(dx: number, dy: number): void {
    ...
    for (const cmd of this.data) {
      if (cmd.type === 'M' || cmd.type === 'L') {
        (cmd as MoveCommand | LCommand).x += dx;
        (cmd as MoveCommand | LCommand).y += dy;
      } else if (cmd.type === 'C') {
        const c = cmd as CCommand;
        c.x1 += dx;
        c.y1 += dy;
        c.x2 += dx;
        c.y2 += dy;
  ...
  public rotate(angle: number): void {
    ...
    for (const cmd of this.data) {
      if (cmd.type === 'M' || cmd.type === 'L') {
        const c = cmd as MoveCommand | LCommand;
  ...
  public update(
    index: number,
    newCmd: MoveCommand | LCommand | CCommand | ZCommand
  ): void {
   ...
   */
  private parsePath(pathString: string): (MoveCommand | LCommand | CCommand | ZCommand)[] {
    const commands: (MoveCommand | LCommand | CCommand | ZCommand)[] = [];
        ...
        switch (cmdLetter.toUpperCase()) {
          case 'M': {
            ...
            if (i + 1 <= tokens.length) {
              ...
              while (i + 1 < tokens.length && !/[MLCZmlcz]/.test(tokens[i])) {
                ...
                i += 2;
                commands.push(new LCommand(lx, ly, rel));
              }
            }
            break;
          }
          case 'L': {
            ...
            while (i + 1 < tokens.length && !/[MLCZmlcz]/.test(tokens[i])) {
              ...
              const y = parseFloat(tokens[i + 1]);
              i += 2;
              commands.push(new LCommand(x, y, rel));
            }
            break;
          }
          case 'C': {
            ...
            while (i + 5 < tokens.length && !/[MLCZmlcz]/.test(tokens[i])) {
              ...
              i += 6;
              commands.push(new CCommand(x1, y1, x2, y2, x, y, rel));
            }
            break;
          }
          case 'Z': {
            const rel = (cmdLetter === 'z');
            commands.push(new ZCommand(rel));
            break;
          }
        }
      } else {
  ...
  convertToAbsolute(): void {
    ...
    for (let i = 0; i < this.data.length; i++) {
      ...
      if (!cmd.relative) {
        // Already absolute
        if (cmd.type === 'M' || cmd.type === 'L') {
          currentX = (cmd as MoveCommand | LCommand).x;
          currentY = (cmd as MoveCommand | LCommand).y;
        } else if (cmd.type === 'C') {
          const c = cmd as CCommand;
          currentX = c.x;
          currentY = c.y;
 ...
 *   index: number;        // index of the closest command in this.data
  ...
  */
  ...
  public split(
    ...
    const cmdToSplit: TSVGCommand = this.data[index];
    ...
    if (cmdToSplit.type === 'L') {
      const lc = cmdToSplit as LCommand;
      // old line: start -> lc.x,lc.y
      // new lines: start -> splitPoint, then splitPoint -> end
      cmd1 = new LCommand(splitPoint.x, splitPoint.y, false);
      cmd2 = new LCommand(lc.x, lc.y, false);
    }
    else if (cmdToSplit.type === 'C') {
      const cc = cmdToSplit as CCommand;
      ...
      cmd1 = new CCommand(c1[1].x, c1[1].y, c1[2].x, c1[2].y, c1[3].x, c1[3].y, false);
      cmd2 = new CCommand(c2[1].x, c2[1].y, c2[2].x, c2[2].y, c2[3].x, c2[3].y, false);
    }
    else {
      throw new Error(`split: Unknown command type ${(cmdToSplit as TSVGCommand).type}`);
    }

    // Remove the old command from the data array
    this.data.splice(index, 1, cmd1 as TSVGCommand, cmd2 as TSVGCommand);
  ...
  private getSegmentStart(index: number): { startX: number; startY: number } {
    ...
    for (let i = 0; i < index; i++) {
      ...
      if (c.type === 'M') {
        ...
        const l = c as LCommand;
        currX = l.x;
        currY = l.y;
      }
      else if (c.type === 'C') {
        const cc = c as CCommand;
        currX = cc.x;
        currY = cc.y;
      }
      else if (c.type === 'Z') {
