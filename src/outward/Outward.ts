...
export function createExternalOutwardPolygon(vector: string, distance: number, precision: number = 15) {
  const poly = syncCreatePolygonFromSVGPaths(vector, precision);
  const polyPath = poly.map((p) => "M" + p.map(_ => _.join(" ")).join("L")).join("Z");


  const out = offsetPolygon(poly[0].map(_ => new Point(_[0], _[1])), distance, true);
  const outward = checkIntersect(out);

  let path = "M" + outward[0].x + " " + outward[0].y + " ";
...
export function offsetPolygon(points: Point[], distance: number, outward: boolean = true): Point[] {
  ...
  for (let i = 0; i < n; i++) {
    ...
    const _offsetPoint = offsetPoint(prev, current, next, distance, outward);
    offseted.push(_offsetPoint);

    // Debug: Log each offset point for verification
  }

  // Ensure closed loop by connecting back to the first point
  offseted.push(offseted[0]);
...
export function checkIntersect(polygon: Point[]) {
  ...
  // Collect all intersections without modifying the polygon
  for (let i = 0; i < n; i++) {
    const p1 = polygon[i];
    const p2 = polygon[(i + 1) % n];

    for (let j = 0; j < n; j++) {
      if (Math.abs(i - j) <= 1 || Math.abs(i - j) === n - 1) continue; // Skip adjacent edges
      const p3 = polygon[j];
      const p4 = polygon[(j + 1) % n];

      const intersectPoint = intersect(p1, p2, p3, p4);
      if (intersectPoint) {
        intersections.push({ index: i + 1, point: intersectPoint });
      }
    }
  }

  // Modify the polygon based on intersections found
  for (const { index, point } of intersections) {
